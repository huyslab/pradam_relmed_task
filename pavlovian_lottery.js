...
function initPavlovianLottery() {
  ...
  const prepilt_preload = {
    ...
    auto_preload: true,
    images: [
      [1, 2, 3, 4, 5, 6].map(i => `imgs/Pav_stims/${window.sequenceKey}/PIT${i}.png`),
      [
        "50pence.png",
        "1pound.png",
        "1penny.png",
  ...
  const lotteryAnimation = {
    ...
    on_load: function () {
      const currentStimulus = "imgs/Pav_stims/" + window.sequenceKey + "/" + PREPILT_CONFIG.stimulus[jsPsych.evaluateTimelineVariable('pav_value')];
      ...
      const pavlovianStimuli = Object.values(PREPILT_CONFIG.stimulus).map((filename) => {
        return {
          stimulus: "imgs/Pav_stims/" + window.sequenceKey + "/" + filename
        };
      });
  ...
  const showResult = {
    type: jsPsychHtmlKeyboardResponse,
    stimulus: function () {
      const stimulusImg = "imgs/Pav_stims/" + window.sequenceKey + "/" + PREPILT_CONFIG.stimulus[jsPsych.evaluateTimelineVariable('pav_value')];
