...
function check_fullscreen(){
    if (window.debug || window.context === "pradam"){
        return false
    }
...
function saveDataREDCap(retry = 1, extra_fields = {}, callback = () => {}) {
    ...
    if (window.context === "pradam") {
        ...
        throw new Error(`This program is not ready for prolific. Please check context. Current context: ${window.context}`);
...
const bonus_trial = {
    ...
    stimulus: function (trial) {
        let event = window.context === "pradam" ? "module" : "session";
